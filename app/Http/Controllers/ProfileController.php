...
class ProfileController extends Controller
{
    ...
    public function saveHobbies(Request $request, $username){
        ...
        $request->session()->flash('alert-success', ' mis à jour avec succès.!');
    ...
    public function saveRelationship(Request $request, $username){
        ...
        if ($relation->save()) {
            ...
            $request->session()->flash('alert-success', 'Votre relation a été demandée avec succès! Il / Elle doit accepter la relation avec vous pour publier.');

        }else{
            $request->session()->flash('alert-danger', 'Quelque chose a mal tourné!');
        }
    ...
    public function uploadCover(Request $request, $username){
        ...
        if ($validator->fails()) {
            ...
            if ($file->storeAs('public/uploads/covers', $file_name)){
                ...
                $response['code'] = 400;
                $response['message'] = "Quelque chose a mal tourné!";
            }
        }
        return Response::json($response);
