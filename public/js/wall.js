    ...
    if (!fetch_end) {
        ...
        $.ajax({
            ...
            error: function () {
                /*
                $('#errorMessageModal').modal('show');
                $('#errorMessageModal #errors').html('Quelque chose s'est mal passé lors du chargement de votre mur!');*/
                $('.post-list-top-loading').hide();
                $('.post-list-bottom-loading').hide();
                fetch_end = false;
            }
...
function deletePost(id){
    ...
    BootstrapDialog.show({
        title: 'Supprimer le publication!',
            ...
            action: function(dialog) {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        dialog.close();
                        if (response.code == 200){
                            $('#panel-post-'+id).html("Pubs supprimé!");
                        }else{
                            $('#errorMessageModal').modal('show');
                            $('#errorMessageModal #errors').html('Une erruer !');
                        }
                    },
                    error: function(){
                        dialog.close();
                        $('#errorMessageModal').modal('show');
                        $('#errorMessageModal #errors').html('Une erreur !');
                    }
                });
            }
        }, {
...
function likePost(id){
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.code == 200){
                if (response.type == 'like'){
                    ...
                    $('#panel-post-'+id+' .like-text span').html('aimer!');
                    $('#panel-post-'+id+' .like-text i').removeClass('fa-heart').addClass('fa-heart-o');
                }
                if (response.like_count > 1){
                    $('#panel-post-'+id+' .all_likes span').html(response.like_count+' aimer');
                }else{
                    $('#panel-post-'+id+' .all_likes span').html(response.like_count+' aimer');
                }
            }else{
        ...
        error: function(){
            $('#errorMessageModal').modal('show');
            $('#errorMessageModal #errors').html('Arrivé!');
        }
    });
}
...
function submitComment(id){
    ...
    if (comment.trim() == ''){
        $('#errorMessageModal').modal('show');
        $('#errorMessageModal #errors').html('Rédiger un commentaire!');
    }else {
        $.ajax({
            ...
            action: function(dialog) {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        dialog.close();
                        if (response.code == 200){
                            $('#post-comment-'+id+' .panel-body').html("<p><small>Commentaire supprimé!</small></p>");
